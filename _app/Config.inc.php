...
define('SIS_DB_USER', 'donnaglam'); //Usuário do banco de dados
define('SIS_DB_PASS', 'c0ntr0l3t0t@l'); //Senha  do banco de dados
define('SIS_DB_DBSA', 'donnaglam_ecommerce'); //Nome  do banco de dados
/*
 * CACHE E CONFIG
 */
define('SIS_CACHE_TIME', 10); //Tempo em minutos de sessão
define('SIS_CONFIG_WC', true); //Registrar configurações no banco para gerenciar pelo painel!
/*
 * AUTO MANAGER
 */
 ...
 */
$CastelTecDefineConf  = null;
if (SIS_CONFIG_WC) :
    $Read = new Read;
    $Read->FullRead("SELECT conf_key, conf_value FROM " . DB_CONF);
    if ($Read->getResult()) :
        foreach ($Read->getResult() as $CastelTecDefineConf) :
            define("{$CastelTecDefineConf['conf_key']}", "{$CastelTecDefineConf['conf_value']}");
        endforeach;
        $CastelTecDefineConf  = true;
    endif;
endif;
