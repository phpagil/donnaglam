...
elseif ($CommentType == 'product'):
    ...
    echo "<section class='comments' id='comments'>";
    echo "<header><h1>{$CommentTitle}<span>Confira as opiniões e avaliações de nossos clientes</span></h1></header>";
endif;

if ($Read->getResult()):
    echo "<div class='comments_count'>{$CommentCount} comentário(s) - NÃO SE ESQUEÇA DE DEIXAR O SEU :</div>";
    foreach ($Read->getResult() as $Comment):
        ...
        if (empty($LikeThisPost)):
            echo "<span class='like wc_like' id='{$Comment['id']}'><b>{$LikeCount}</b> Curtir </span>";
        else:
            $LikeThisPost = null;
        ...
        if ($Read->getResult()):
            foreach ($Read->getResult() as $Response):
                ...
                if (empty($LikeThisPost)):
                    echo "<span class='like wc_like' id='{$Response['id']}'><b>{$LikeCount}</b> Curtir </span>";
                else:
                    $LikeThisPost = null;
                    echo "<span class='liked'><b>{$LikeCount}</b> VOCÊ JÁ CURTIU ISSO </span>";
                endif;
