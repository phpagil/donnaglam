...
if (!$OrId):
    ...
    if (!$Read->getResult()):
        ...
        echo "<p style='width: 100%;'><b>Postado dia: </b>" . ($order_shipment ? date('d/m/Y', strtotime($order_shipment)) : 'Aguardando envio!') . " " . ($order_tracking && $order_tracking != 1 ? "- <a class='font_blue' target='_blank' href='{$ShipmentUrl}{$order_tracking}' title='Rastrear Pedido'>Acompanhar Envio!</a>" : "") . "</p>";
        ...
        elseif ($order_status != 1 && $order_status != 6 && date('Y-m-d H:i:s', strtotime($order_date . "+" . E_ORDER_DAYS . "days")) > date('Y-m-d H:i:s')):
