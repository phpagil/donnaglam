...
switch ($Action):
    ...
    case 'creditCard':
        ...
        $CastelTecPayMentRequest = new PagSeguroDirectPaymentRequest();
        $CastelTecPayMentRequest->setPaymentMode('DEFAULT');
        $CastelTecPayMentRequest->setPaymentMethod('CREDIT_CARD');
        $CastelTecPayMentRequest->setCurrency("BRL");
        $CastelTecPayMentRequest->setReference($_SESSION['wc_payorder']['order_id']);
        $CastelTecPayMentRequest->addParameter("notificationURL", BASE . "/_cdn/widgets/ecommerce/PagSeguro/PagSeguroNotify.workcontrol.php");
        ...
        $CastelTecPayMentRequest->setSender("{$PayMer['user_name']} {$PayMer['user_lastname']}", $PayMer['user_email'], substr($PayMer['user_cell'], 1, 2), substr(str_replace('-', '', $PayMer['user_cell']), 5), 'CPF', str_replace("-", "", $PayMer['user_document']));
        $CastelTecPayMentRequest->setSenderHash($POST['senderHash']);

        //ADDR
        $Read->ExeRead(DB_USERS_ADDR, "WHERE addr_id = :addid", "addid={$_SESSION['wc_payorder']['order_addr']}");
        $PayAddr = $Read->getResult()[0];
        $CastelTecPayMentRequest->setShippingAddress(str_replace('-', '', $PayAddr['addr_zipcode']), $PayAddr['addr_street'], $PayAddr['addr_number'], $PayAddr['addr_complement'], $PayAddr['addr_district'], $PayAddr['addr_city'], $PayAddr['addr_state'], $PayAddr['addr_country']);

        //SHIP
        $ShipmentCost = (!empty($_SESSION['wc_payorder']['order_shipprice']) ? number_format($_SESSION['wc_payorder']['order_shipprice'], '2', '.', '') : 0);
        $CastelTecPayMentRequest->setShippingType(3); //Frete
        $CastelTecPayMentRequest->setShippingCost($ShipmentCost);

        //ORDER ITEM IDENTIFY
        $CastelTecPayMentRequest->addItem($_SESSION['wc_payorder']['order_id'], "Pedido #" . str_pad($_SESSION['wc_payorder']['order_id'], 7, 0, 0), 1, number_format($_SESSION['wc_payorder']['order_price'] - $ShipmentCost, '2', '.', ''));
        ...
        //ORDER SUBMIT
        $CastelTecPayMentRequest->setCreditCard($creditCardData);
        try {
            $credentials = PagSeguroConfig::getAccountCredentials();
            $response = $CastelTecPayMentRequest->register($credentials);
            $UpdateOrder = [
                'order_payment' => 101,
            ...
            if (ECOMMERCE_STOCK == 'payment'):
                $Read->FullRead("SELECT pdt_id, pdt_inventory, pdt_delivered FROM " . DB_PDT . " WHERE pdt_id IN(SELECT pdt_id FROM " . DB_ORDERS_ITEMS . " WHERE order_id = :ord AND pdt_id IS NOT NULL)", "ord={$_SESSION['wc_payorder']['order_id']}");
                if ($Read->getResult()):
                    foreach ($Read->getResult() as $InventoryPdt):
                        $Read->FullRead("SELECT item_amount FROM " . DB_ORDERS_ITEMS . " WHERE order_id = :ord AND pdt_id = :pdt", "ord={$_SESSION['wc_payorder']['order_id']}&pdt={$InventoryPdt['pdt_id']}");
                        if ($Read->getResult()):
                            $UpdateProduct = [
    ...
    case 'billet':
        if (empty($_SESSION['wc_payorder'])):
            ...
            $CastelTecPayMentRequest = new PagSeguroDirectPaymentRequest();
            $CastelTecPayMentRequest->setPaymentMode('DEFAULT');
            $CastelTecPayMentRequest->setPaymentMethod('BOLETO');
            $CastelTecPayMentRequest->setExtraAmount('-1.00');
            $CastelTecPayMentRequest->setCurrency("BRL");
            $CastelTecPayMentRequest->setReference($_SESSION['wc_payorder']['order_id']);
            $CastelTecPayMentRequest->addParameter("notificationURL", BASE . "/_cdn/widgets/ecommerce/PagSeguro/PagSeguroNotify.workcontrol.php");
