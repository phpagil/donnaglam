...
switch ($Action):
    ...
    case 'cart_shipment':
        ...
        $data['nVlAltura'] = $pdt_dimension_heigth;
        ...
        $data2['nCdEmpresa'] = ECOMMERCE_SHIPMENT_CDEMPRESA;
        $data2['sDsSenha'] = ECOMMERCE_SHIPMENT_CDSENHA;
        $data2['sCepOrigem'] = str_replace('-', '', SITE_ADDR_ZIP);
        $data2['sCepDestino'] = str_replace('-', '', $POST['zipcode']);
        $data2['nVlPeso'] = $WeightTotalShip;
        $data2['nCdFormato'] = ECOMMERCE_SHIPMENT_FORMAT;
        $data2['nVlComprimento'] = $DepthTotalShip;
        $data2['nVlAltura'] = $pdt_dimension_heigth;
        $data2['nVlLargura'] = $WidthTotalShip;
        $data2['nVlDiametro'] = '0';
        $data2['sCdMaoPropria'] = 's';
        $data2['nVlValorDeclarado'] = (ECOMMERCE_SHIPMENT_DECLARE ? $CartTotalShip : '0');
        $data2['sCdAvisoRecebimento'] = (ECOMMERCE_SHIPMENT_ALERT ? 's' : 'n');
        $data2['StrRetorno'] = 'xml';
        $data2['nCdServico'] = '41106';
        $data2 = http_build_query($data2);

        $url = 'http://ws.correios.com.br/calculador/CalcPrecoPrazo.aspx';
        $curl = curl_init($url . '?' . $data2);
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
        $result = curl_exec($curl);
        $resultXml2 = simplexml_load_string($result);
        ...
        foreach ($resultXml2->cServico as $row) :
            if ($row->Erro == 0) :
                $jSON['cart_shipment'] .= "<label class='shiptag'><input required class='wc_shipment' name='shipment' value='" . str_replace(',', '.', $row->Valor) . "' type='radio' id='{$row->Codigo}'/> " . getShipmentTag(intval($row->Codigo)) . ": 01 a " . str_pad($row->PrazoEntrega + ECOMMERCE_SHIPMENT_DELAY, 2, 0, 0) . " dias úteis - R$ {$row->Valor}</label>";
            endif;
        endforeach;
        ...
        // SHIPMENT_LOCAL 02
        if (ECOMMERCE_SHIPMENT_LOCAL_2) :
            $City = json_decode(file_get_contents("https://viacep.com.br/ws/" . str_replace('-', '', $POST['zipcode']) . "/json/"));
            if (!empty($City) && !empty($City->localidade) && $City->localidade == ECOMMERCE_SHIPMENT_LOCAL_2) :
                $jSON['cart_shipment'] = "<label class='shiptag'><input required class='wc_shipment' name='shipment' value='" . ECOMMERCE_SHIPMENT_LOCAL_PRICE_2 . "' type='radio' id='10004'/> Taxa de entrega: R$ " . number_format(ECOMMERCE_SHIPMENT_LOCAL_PRICE_2, 2, ',', '.') . "</label>";
            endif;
        endif;


        if (empty($jSON['cart_shipment']) && empty($ErroZip)) :
            $jSON['trigger'] = AjaxErro("<b>Oops:</b> Não existem opções de entrega para o pedido autal. Você pode remover ou adicionar alguns produtos para tentar novamente!<p>Ou caso queira, entre em contato para que possamos te ajudar!</p><p>Fone: " . SITE_ADDR_PHONE_A . "<br>E-mail: " . SITE_ADDR_EMAIL . "</p>", E_USER_WARNING);
