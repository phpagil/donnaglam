$(function () {
  var action =
    $('link[rel="base"]').attr('href') +
    '/_cdn/widgets/ecommerce/cart.ajax.php';
  ...
  $('.wc_cart_remove').click(function () {
    ...
    $.post(
      action,
      { action: 'wc_cart_remove', pdt_id: pdt_id },
      function (data) {
  ...
  $('.wc_cart_change_plus').click(function () {
    ...
    if (
      ...
      wcCartTrigger(
        "<div class='trigger trigger_info'><b>OPPSSS:</b> Desculpe, mas só existem " +
          input.attr('max') +
          ' unidades deste produto em estoque!</div>'
      );
    }
    ...
    $.post(
      action,
      { action: 'wc_cart_change', pdt_id: pdt_id, item_amount: input.val() },
      function (data) {
        $('.wc_cart_total span').html(data.cart_total);
        $('.wc_cart_price span').html(data.cart_price);
  ...
  $('.wc_cart_plus').click(function () {
    ...
    if (parseInt(Amount) <= parseInt(Input.attr('max')) || !Input.attr('max')) {
      ...
      Form.find('button')
        .last()
        .after(
          "<p class='m_top ds_none trigger trigger_info none'><b>OPPSSS:</b> Temos " +
            Input.attr('max') +
            ' unidades em estoque!'
        );
  ...
  $('.wc_cart_cupom').click(function () {
    ...
    if (!input.val()) {
      wcCartTrigger(
        "<div class='trigger trigger_info'><b>OPPSSS:</b> Informe o código do cupom para aplicar e aproveitar seu desconto especial!</div>"
      );
    } else {
  ...
  $('.wc_cart_ship').click(function () {
    ...
    if (!input.val()) {
      wcCartTrigger(
        "<div class='trigger trigger_info'><b>OPPSSS:</b> É preciso informar o CEP de destino da encomenda para calcular o frete!</div>"
      );
    } else {
  ...
  $('.wc_order_create').submit(function () {
    ...
    $.post(
      ...
      function (data) {
        ...
        if (data.form_error) {
          if (data.field) {
            Form.find("input[name='" + data.field + "']").after(
              data.form_error
            );
          } else {
  ...
  function wcCartTrigger(Message) {
    if (Message) {
      $('.wc_cart_callback').html('').fadeOut(1).css('z-index', '1000');

      $('.wc_cart_callback').html(Message).fadeIn(400);
      $('.wc_cart_callback').animate({ right: '0', 'z-index': '1000' }, 100);
