<script>
    $(function () {
        ...
        $('.workcontrol_pay_tabs li').click(function () {
            var CastelTecPayTab = $(this).attr('id');
            var CastelTecTab = $(this);
            $('.workcontrol_pay_tabs li').removeClass('active');
            $("form[id!='" + CastelTecPayTab + "']").slideUp(200, function () {
                $("form#" + CastelTecPayTab).slideDown(200);
                CastelTecTab.addClass('active');
            });
        });
        ...
        $("form#card").submit(function () {
            ...
            $.ajax({
                url: '<?= BASE; ?>/_cdn/widgets/ecommerce/cart.pagseguro.php',
                data: Data,
                type: 'POST',
                dataType: 'json',
                beforeSend: function () {
                    ...
                    if (data.error) {
                        ...
                        var param = {
                            ...
                            success: function (data) {
                                ...
                                $.post('<?= BASE; ?>/_cdn/widgets/ecommerce/cart.pagseguro.php', Data, function (data) {
                                    //MODAL ERRORS
            ...
            PagSeguroDirectPayment.getInstallments({
                ...
                success: function (data) {
                    ...
                    $.each(Installments, function (index, elem) {
                        $("#cardInstallmentQuantity option:last").after('<option value="' + elem.quantity + 'x' + elem.installmentAmount + '">' + elem.quantity + 'x R$ ' + number_format(elem.installmentAmount, '2', ',', '.') + ' - ' + (elem.interestFree === true ? "sem acréscicimo" : 'R$ ' + number_format(elem.totalAmount, '2', ',', '.') + '*') + '</option>');
                    });
                }
            });
        }
