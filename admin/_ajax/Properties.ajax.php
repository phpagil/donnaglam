...
if ($PostData && $PostData['callback_action'] && $PostData['callback'] = $CallBack):
    ...
    switch ($Case):
        ...
        case 'manager':
            ...
            if (empty($RealTyCover) && (!$Read->getResult() || !$Read->getResult()[0]['realty_cover'])):
                $jSON['trigger'] = AjaxErro('<b class="icon-warning">ERRO AO CADASTRAR:</b> Favor envie uma imagem de capa do imóvel nas medidas de ' . IMAGE_W . 'x' . IMAGE_H . 'px!', E_USER_WARNING);
            else:
                ...
                $PostData['realty_name'] = Check::Name($PostData['realty_title']);
                ...
                if (!empty($RealTyCover)):
                    ...
                    $Upload->Image($RealTyCover, Check::Name($PostData['realty_title']), IMAGE_W, 'properties');
                ...
                if (!empty($Image)):
                    ...
                    foreach ($gbFiles as $UploadFile):
                        $gbLoop ++;
                        $Upload->Image($UploadFile, "{$PostData['realty_name']}{$RealtyId}-{$gbLoop}-" . time() . base64_encode(time()), IMAGE_W, 'properties');
                        if ($Upload->getResult()):
                            $gbCreate = ['realty_id' => $RealtyId, "image" => $Upload->getResult()];
                            $Create->ExeCreate(DB_IMOBI_GALLERY, $gbCreate);
                            $jSON['gallery'] .= "<img id='{$Create->getResult()}' alt='Imagem em {$PostData['realty_title']}' title='Imagem em {$PostData['realty_title']}' src='../uploads/{$Upload->getResult()}'/>";
                        endif;
                    endforeach;
                endif;
