...
<header class="dashboard_header">
    ...
    <div class="dashboard_header_search" style="font-size: 0.875em; margin-top: 16px;" id="<?= $OrderId; ?>">
        <span rel='dashboard_header_search' class='j_delete_action icon-warning btn btn_red' id='<?= $OrderId; ?>'>Deletar Pedido!</span>
    ...
    <form name="post_create" action="" method="post" enctype="multipart/form-data">
        ...
        <article class="box box30">
            ...
            <div class="box_content">
                ...
                <p class="section icon-location2">Mail Tag:</p>
                <blockquote>
                    <?php
                    $Read->FullRead("SELECT pdt_id, item_name, item_amount FROM " . DB_ORDERS_ITEMS . " WHERE order_id = :order", "order={$OrderId}");
                    if ($Read->getResult()) :
                        ...
                        foreach ($Read->getResult() as $Items) :
                            ...
                            if ($Read->getResult()) :
                                echo "[&nbsp;&nbsp;] {$Items['item_amount']} - ({$Read->getResult()[0]['pdt_code']}) {$Items['item_name']}<br>";
                            else :
                                echo "[&nbsp;&nbsp;] {$Items['item_amount']} - {$Items['item_name']}<br>";
                            endif;
                        endforeach;
            ...
            <div class="box_content">
                <div class="label_50">
                    ...
                    <label class="label">
                        ...
                        <p style="font-size: 1.3em;" class="input icon-history"><?= getOrderStatus($order_status); ?></p>
                    </label>
                </div>
                ...
                <div class="single_order_items">
                    ...
                    if (!$Read->getResult()) :
                        ...
                        foreach ($Read->getResult() as $Item) :
                            ...
                            if ($Item['pdt_id']) :
                                echo "<p><b>" . str_pad($Item['item_id'], 5, 0, STR_PAD_LEFT) . "</b> - <a target='_blanc' href='dashboard.php?wc=products/create&id={$Item['pdt_id']}' title='Ver Produto!'>" . Check::Words($Item['item_name'], 4) . "</a></p>";
                ...
                <div class="label_50">
                    <label class="label">
                        ...
                        <select name="order_payment" required>
                            <option selected="selected" disabled="disabled" value="<?= $order_payment; ?>">&checkmark; <?= getOrderPayment($order_payment); ?></option>
                            <?php
                    ...
                    <label class="label">
                        ...
                        <select name="order_status" required>
                            <option selected="selected" disabled="disabled" value="<?= $order_status; ?>">&checkmark; <?= getOrderStatus($order_status); ?></option>
                ...
                <?php if ($numero_nf == 0) { ?>
                    <a value="1" class="btn btn_red fl_right icon-share" style="margin-left: 5px;" target="_blank" href="/api_ecomerce/order/<?php echo $OrderId ?>">Emitir NF-e!</a>
                <?php } else { ?>
                    <a value="1" class="btn btn_blue fl_right icon-share" style="margin-left: 5px;" target="_blank" href="/api_ecomerce/nfe/imprimirPorNFe/<?php echo $numero_nf ?>">Imprimir NF-e!</a>
                <?php } ?>
