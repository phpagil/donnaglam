...
<div class="dashboard_content">
    ...
    $Read->ExeRead(DB_SLIDES, "WHERE slide_start >= NOW() OR slide_end <= NOW() ORDER BY slide_date DESC LIMIT :limit OFFSET :offset", "limit={$Pager->getLimit()}&offset={$Pager->getOffset()}");
    if (!$Read->getResult()) :
        $Pager->ReturnPage();
        ...
        foreach ($Read->getResult() as $Slide) :
            ...
            echo "<article class='box box100 slide_single' id='{$slide_id}'>
                    ...
                    <img src='" . BASE . "/uploads/{$slide_image}?nocache=" . time() . "' title='{$slide_title}' alt='{$slide_title}'>
