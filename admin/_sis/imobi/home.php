...
<div class="dashboard_content">
    ...
    $Read->ExeRead(DB_IMOBI, "ORDER BY realty_date DESC LIMIT :limit OFFSET :offset", "limit={$Paginator->getLimit()}&offset={$Paginator->getOffset()}");
    if (!$Read->getResult()) :
        ...
        foreach ($Read->getResult() as $REALTY) :
            ...
            $RealTyType = (getWcRealtyType($realty_type) && is_string(getWcRealtyType($realty_type)) ? getWcRealtyType($realty_type) : 'Infefinido');
            ...
            echo "<article class='box box25 imobi_single' id='{$realty_id}'>
                <div class='post_single_cover'>
                    <img alt='{$realty_title}' title='{$realty_title}' src='../tim.php?src={$RealTyCover}&w=" . IMAGE_W . "&h=" . IMAGE_H . "'/>
                    <div class='post_single_status'><span class='btn'>" . str_pad($realty_views, 4, 0, STR_PAD_LEFT) . "</span>{$RealTyStatus}</div>
                    <div class='post_single_cat'>{$RealTyType} " . (is_string(getWcRealtyFinality($realty_finality)) ? getWcRealtyFinality($realty_finality) : '') . ($realty_observation ? " - <b>{$realty_observation}</b>" : null) . "</div>
                </div>
                <div class='box_content'>
                    ...
                    <h1 class='title'>" . Check::Chars($realty_title, 40) . " <b class='font_yellow'> " . ($realty_price ? "R$&nbsp;" . number_format($realty_price, 2, ',', '.') : 'A Combinar') . "</b></h1>
                    ...
                    <div>&nbsp;</div>
                    <a title='Ver imóvel no site' target='_blank' href='" . BASE . "/imovel/{$realty_name}' class='icon-notext icon-eye btn btn_green'></a>
                    <a title='Editar Imóvel' href='dashboard.php?wc=imobi/create&id={$realty_id}' class='post_single_center icon-notext icon-pencil btn btn_blue'></a>
                    <span rel='imobi_single' class='j_delete_action icon-notext icon-cancel-circle btn btn_red' id='{$realty_id}'></span>
        ...
        $Paginator->ExePaginator(DB_IMOBI);
